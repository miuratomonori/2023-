...
crossorigin="anonymous"
    ...
    let top_arry = [];
    let year_top;
    let month_top = [];     //0に年を、1に月を入れる
    let day_top;

    //last側の変数
    let last_arry = [];
    let year_last;
    let month_last = [];    //0に年を、1に月を入れる
    let day_last;
    ...
    x_date_top.onchange=function(){
        ...
        for(var i=0; i < top_arry.length; i++){
            if(i == 0 && top_arry[i] >= 1946 && top_arry[i] <= 2022 && top_arry[i].match(/^[0-9]*$/) != null){
                year_top = top_arry[i];
            }
            else if(i == 1 && top_arry[i] >= 1 && top_arry[i] <= 12 && top_arry[i].match(/^[0-9]*$/) != null){
                month_top = top_arry[i];
            }
